...
async function main() {
  ...
  // Galleries kept off the site stay in the bucket but leave the manifest here, so
  // neither the sitemap, the snapshot nor the same-origin copy carry them. config.ts
  // applies the same lists to the live manifest at runtime, reading this build's
  // target from src/app/generated/site-target.ts, which this script writes.
  //
  // SITE_TARGET=live (what `npm run deploy` sets) also drops the liveSiteOnly lists;
  // a plain run - `npm run sitemap`, the preview channel - keeps them, so the site
  // can be reviewed whole while the deployed one waits for consent.
  const hiddenConfig = JSON.parse(await readFile(join(REPO_ROOT, 'src', 'app', 'content', 'hidden-galleries.json'), 'utf8'));
  const live = process.env.SITE_TARGET === 'live';
  const hiddenGalleries = [...hiddenConfig.everywhere, ...(live ? hiddenConfig.liveSiteOnly.galleries : [])]
    .map((prefix) => prefix.normalize('NFC'));
  const hiddenTypes = live ? hiddenConfig.liveSiteOnly.types : [];
  console.log(`Target: ${live ? 'live site' : 'preview'}`);
  for (const prefix of Object.keys(manifest.galleries)) {
    if (hiddenGalleries.includes(prefix.normalize('NFC')) || hiddenTypes.includes(prefix.slice(0, prefix.indexOf('/')))) {
      delete manifest.galleries[prefix];
      delete manifest.dims?.[prefix];
      console.log(`  - hidden: ${prefix}`);
    }
  }
  await writeFile(join(REPO_ROOT, 'src', 'app', 'generated', 'site-target.ts'), [
    ...
    `export const HIDDEN_TYPES: string[] = ${JSON.stringify(hiddenTypes)};`,
    `export const HIDDEN_GALLERIES: string[] = ${JSON.stringify(hiddenGalleries)};`,
    '',
  ].join('\n'), 'utf8');
